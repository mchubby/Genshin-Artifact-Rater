...
def api_upload():
	...
	if suc:
		...
		return jsonify({
			...
			'reason': '',
			'score': score,
			'rating': get_humanized_rating(score),
...
labels = [
	'FODDER:This is unadulterated trash',
	'AMBER:You could make something of it, but is it worth your time?',
	'MEH:Meh.',
	'CHILDE:Good while it lasts',
	'COCOGOAT:You didn\'t photoshop that, did you?',
]
ranges = np.delete(np.linspace(0, 100, num=6), 0)


def get_humanized_rating(score):
	# find nearest index in array, zero-based and map to labels
	return labels[(np.abs(ranges - score)).argmin()]


async def ocr_from_flask_blob(num, lang=tr.en()):
	if not OCR_API_KEY:
	...
	async with aiohttp.ClientSession() as session:
		...
		data.add_field('file', posting_img.tobytes(), content_type='image/png', filename='image.png')
